...
using MonoTorrent;
using NovumPatchServer.Constants;

namespace NovumPatchServer.Controllers;
...
public class PatchController : Controller
{
    ...
    private void SetHeaders(bool isUpdate, string type, string latestVersion)
    {
        ...
        if (isUpdate) headerDictionary.Add("Connection", "keep-alive");

        foreach (var VARIABLE in headerDictionary) Response.Headers.Add(VARIABLE);
    ...
    private async Task<bool> SetBody(string type, string currentVersion, Dictionary<string, string> versions,
